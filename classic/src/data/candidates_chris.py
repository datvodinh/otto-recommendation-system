...
def matrix_to_candids_dict(matrix):
    ...
    Returns:
        dict: Dictionary mapping aids to their candidate lists.
    """
    try:
        # Sort values by weight in descending order
        candids = matrix.sort_values("wgt", ascending=False)
    except AttributeError:
        # Handle case where sorting fails
        candids = matrix

    # Group by aid_x and aggregate aid_y into lists
    try:
        candids = candids.groupby("aid_x").agg({"aid_y": list}).reset_index()
    except AttributeError:
        pass

    # Convert aid_y column values to lists if they're numpy arrays
    # candids["aid_y"] = candids["aid_y"].apply(
...
def load_parquets(regex):
    ...
    return (
        cudf.concat(dfs)
        .sort_values(["session", "ts"], ignore_index=True)
        .to_pandas()
    )
...
def explode(df, test=False):
    ...
    if not test:
        for col in ["gt_clicks", "gt_carts", "gt_orders"]:
            ...
            df_tgt[col] = df_tgt[col].astype("int64") == df_tgt[
                "candidates"
            ].astype("int64")
            assert not df_tgt.isna().any().max()

            df_tgt = (
                df_tgt.groupby(["session", "candidates"]).max().reset_index()
            )
